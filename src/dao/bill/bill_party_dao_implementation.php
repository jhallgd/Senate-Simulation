...
class bill_party_dao_implementation implements bill_party_dao_interface {
    ...
    public function find_by_id($bill_id): bills_parties{
        $sql = "SELECT pb_id, bl_id, bl_title, bl_short_text, bl_url, pa_id, pa_name, pvt_id, pvt_view, pvt_color   
		FROM PartiesBills
    ...
    public function update(int $pb_id, int $pvt_id):bool{
        $sql = 'UPDATE PartiesBills SET pb_pvt_id = '. $pvt_id . ' WHERE pb_id = '.$pb_id.';';
        return $this->db->run_transaction([$sql]);
    }
}
